...
"""Larnitech lamp `dehumidifier` sub-type mapped to humidifier (on/off)."""
...
def _claims(device: dict) -> bool:
    return device.get("type") == "lamp" and lamp_platform(device) == "humidifier"
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    def _add_new():
        ...
        new = [LarnitechDehumidifier(coordinator, a) for a in current - known]
        known.clear()
...
class LarnitechDehumidifier(LarnitechEntity, HumidifierEntity):
