...
- **HA-сервер: Popovych** — тестовый HA для проверки Larnitech. Любую версию
  заливаем сюда. Доставка — **по SSH** (порт `11022`, см. `access/home-popovych.md`
  в базе знаний). Не клиентский прод — можно ломать/перекатывать свободно.
- **Larnitech-контроллер: `411317ac`** — тестовый стенд (cloud), на нём можно
  переключать устройства для проверки чтения/записи/событий.
...
| Деплой на Popovych | `scp` по SSH в `/config/custom_components/larnitech` + `ha core restart` |
